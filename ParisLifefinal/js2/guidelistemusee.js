function getidmusee(id){
	
	localStorage.setItem("guideitem",localStorage.getItem("musee"+id));
	localStorage.setItem("cpt","non");
	//alert(localStorage.getItem("final"));
	window.location.assign("./affichedetails.html");
	//alert(localStorage.getItem("guideitem"));
}
...
window.addEventListener('load', function(){
   ...
   // alert(guidetype);
 	...
 	var service="https://maps.googleapis.com/maps/api/place/nearbysearch/json?location="+localStorage.getItem("mypositionlat")+","+localStorage.getItem("mypositionlon")+"&rankby=distance&types=museum&key=AIzaSyBF0ZDR7QPOhG0GD2vuQOdmt9akEk7lhMM"
 	var output = $.ajax({
	    ...
	    success: function(data) {
	    	...
	    	$.each(data.results, function(key, value) {
				...
				localStorage.setItem("musee"+i, JSON.stringify(value));
				...
				*/
				...
				if(value.photos!=null){
					...
					var musee ='<li class="ui-li-has-thumb" id="'+i+'"  onclick="getidmusee(this.id); return false;">'+
