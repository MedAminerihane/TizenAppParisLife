...
function getidbar(id){
	
	localStorage.setItem("guideitem",localStorage.getItem("bar"+id));
...
window.addEventListener('load', function(){
   ...
   // alert(guidetype);
 	...
 	var service="https://maps.googleapis.com/maps/api/place/nearbysearch/json?location="+localStorage.getItem("mypositionlat")+","+localStorage.getItem("mypositionlon")+"&rankby=distance&types=bar&key=AIzaSyBF0ZDR7QPOhG0GD2vuQOdmt9akEk7lhMM"
 	var output = $.ajax({
	    ...
	    success: function(data) {
	    	...
	    	$.each(data.results, function(key, value) {
				...
				localStorage.setItem("bar"+i, JSON.stringify(value));
				...
				*/
				...
				if(value.photos!=null){
					...
					var bar ='<li class="ui-li-has-thumb" id="'+i+'"  onclick="getidbar(this.id); return false;">'+
